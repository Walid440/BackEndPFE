...
// Exemple d'utilisation
  ...
  onSubmit() {
    ...
    const article = this.editForm.value;
    /* 
     this.personl.role=this.editForm.value.role;
    console.log(this.Person);
    formData.append('personnel',JSON.stringify(article));
    formData.append('file',this.userFile);  
    this.Person.EditPerson(this.personl).subscribe( data => {   
      this.Loading();
  ...
  setTimeout (() => {
    this.edited=false;
    Swal.fire("Element enregistré avec succes",'success');
  }, 5200);
